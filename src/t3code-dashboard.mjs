...
async function publishedUrl() {
  try {
    const runtime = JSON.parse(
      await readFile(`${process.env.HOME}/.t3/userdata/server-runtime.json`, 'utf8'),
    )
...
async function createAdministrativePairingLink({ port }) {
  ...
  const logPath = `${process.env.HOME}/.local/state/t3code-host/t3code.log`
...
const ACTIONS = {
  ...
  update: async () => {
    const channel = CHANNEL || channelFor(await installedVersion())
    const { stdout } = await run(NPM_BIN, ['i', '-g', `t3@${channel}`], {
      timeout: 300_000,
    })
...
<main>
  ...
  <div class="card">
    ...
    <div class="fields">
      ...
      <input id="pair-port" aria-label="HTTPS port" type="number" min="1" max="65535" value="443">
    </div>
    <div class="actions">
